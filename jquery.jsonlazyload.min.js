// (c) 2015 Joe Ridino
(function($){'use strict';var JSONLazyLoad=function(element,options){var default_settings={behavior:{ensure_scrollbar:false,fetch_amount:10,fetch_immediate:true,initial_offset:0,loader_delay:100,scroll_threshold:0},files:{pattern:'',num_files:0},elements:{scroll_target:$(window),loader:null},results:{before_callback:null,fail_callback:null,item_callback:null,item_key:null},ws:{data_type:'json',limit_param:'limit',offset_param:'offset',url:''}},added_event_handlers=false,ajaxing=false,check_add_event_handlers,files_offset=0,has_scrollbar,init,jquery_element=$(element),loaded_all=false,loader_hide,loader_show,loader_timeout_id,offset=0,prev_scroll_top=0,promise=null,promise_always,promise_done,promise_fail,resize_check,scroll_check,self=this,settings=null,tear_down;this.target=function(){return jquery_element;};this.fetch_items=function(limit){if(ajaxing||loaded_all){return;}
...
if(settings.behavior.ensure_scrollbar){self.ensure_scrollbar();}};check_add_event_handlers=function(){if(!added_event_handlers){settings.elements.scroll_target.on('scroll',scroll_check);$(window).on('resize',resize_check);added_event_handlers=true;}};promise_done=function(data){if(settings.results.item_key){data=data[settings.results.item_key];}
if(!$.isArray(data)){return;}
offset+=data.length;if(settings.files.pattern){files_offset+=1;}
if(data.length){settings.results.item_callback(self,data);}else{loaded_all=true;tear_down();}
...
if(!loaded_all&&status==='success'&&settings.behavior.ensure_scrollbar){self.ensure_scrollbar();}};loader_show=function(){settings.elements.loader.show();};loader_hide=function(){clearTimeout(loader_timeout_id);settings.elements.loader.hide();};init=function(){settings=$.extend({},default_settings,options);offset=settings.behavior.initial_offset;if(settings.behavior.fetch_immediate){self.fetch_items(settings.behavior.fetch_amount);}else{check_add_event_handlers();}};init();};$.fn.jsonlazyload=function(options){return this.each(function(){var jsonlazyload=new JSONLazyLoad(this,options);$(this).data('jsonlazyload',jsonlazyload);});};}(jQuery));